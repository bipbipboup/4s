...
CTClientMAP::CTClientMAP()
{
	...
	for(int i=0; i<TDRAWCELL_COUNT; i++)
	{
							  ...
							  LPTSNDBLOCK pTSND)
{
	...
	for( int i=0; i<INT(pTMAPNPC->size()); i++)
	{
		...
		// ����ID�� �ΰ��� ���� ������ m_bCountryID�� �߸����� NPC���� ���
		// �ش� ����ID(������,ũ���ÿ�)�� ���õǱ� ������ ���� �߸����� NPC�� ��츦 �˱� ���ؼ� �������� ����ID�� ���� �����Ѵ�.
		pINST->m_bOriginalContryID = (*pTMAPNPC)[i]->m_bCountryID;
		pINST->m_bContryID = (*pTMAPNPC)[i]->m_bCountryID; // <- ��� ���Ŀ� ��Ŷ�� ���ؼ� �ٽ� ���� �� ���̴�.
							   ...
							   LPTSNDBLOCK pTSND)
{
	...
	for(int i=0; i<pTNPC->m_wCount; i++)
	{
	...
	for(int i=0; i<pTSFX->m_wCount; i++)
	{
	...
	for(int i=0; i<pTSND->m_wCount; i++)
	{
							  ...
							  CTRSCSDlg *pTRSCS)
{
	...
	for(int i=0; i<BYTE(pTRSCS->m_vTCORPS.m_vTSQUAD.size()); i++)
		if( pTRSCS->m_vTCORPS.m_vTSQUAD[i]->m_wPartyID == pTRSCS->m_pHost->m_wPartyID || pTRSCS->m_pHost->m_dwChiefID == pTRSCS->m_pHost->m_dwID )
			for( BYTE j=0; j<BYTE(pTRSCS->m_vTCORPS.m_vTSQUAD[i]->m_vTUNIT.size()); j++)
				if( pTRSCS->m_vTCORPS.m_vTSQUAD[i]->m_vTUNIT[j]->m_wMapID == pTRSCS->m_wMapID &&
					pTRSCS->m_vTCORPS.m_vTSQUAD[i]->m_vTUNIT[j]->m_dwHP && (
								 ...
								 CTClientCAM *pCamera)
{
	...
	for(int i=0; i<WORD(vCORPS.size()); ++i )
	{
								  ...
								  LPTLOADOBJ pTINFO)
{
	...
	for(int i=0; i<INT(pTINFO->m_vUnitID.size()); i++)
	{
	...
	for(int i=0; i<INT(pTINFO->m_vTBSPINFO.size()); i++)
	{
